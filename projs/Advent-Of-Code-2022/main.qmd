...
simulate_move <- function(DT,
  ...
  for(i in RowsToWalk){
    ...
    distance <- c(x = DT$x_H_pos[i] - DT$x_T_old_pos[i],
                  y = DT$y_H_pos[i] - DT$y_T_old_pos[i])
    
    # Don't move
    if(all(abs(distance) <= 1)){
      set(DT,i,"y_T_new_pos",DT$y_T_old_pos[i])
      set(DT,i,"x_T_new_pos",DT$x_T_old_pos[i])
      
      # Move TR  
    }else if(distance["x"] >= 1 & distance["y"] >= 1){
      set(DT,i,"y_T_new_pos",DT$y_T_old_pos[i] + 1)
      set(DT,i,"x_T_new_pos",DT$x_T_old_pos[i] + 1)  
...
BaseTable1[, unique(.SD),
          .SDcols = c("x_T_new_pos","y_T_new_pos")
][, .N]
...
BaseTable2[, unique(.SD),
          .SDcols = c("x_9_new_pos","y_9_new_pos")
][, .N]
