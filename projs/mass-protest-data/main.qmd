...
```{r}
get_percentile_ci <- function(x, conf_level = 0.95, na.rm = TRUE){
  
  percentile1 <- (1-conf_level)/2
  percentile2 <- percentile1 + conf_level
  
  data.table(lower_ci = quantile(x, probs = percentile1, na.rm = na.rm, names =FALSE),
             point_estimate = median(x, na.rm = na.rm),
             upper_ci = quantile(x, probs = percentile2, na.rm = na.rm, names =FALSE))
}
```
...
### Are some responses more often in countries with high polity?
...
ProtestLocationExtracted[!is.na(polity), 
    ...
    get_percentile_ci(),
  by = "Response"
][, Response := fct_reorder(Response, point_estimate, sum)] |>
  ggplot(aes(point_estimate , Response))+
  ...
  labs(title = "State Response vs Polity Level",
       y = "State Response", 
       x = "Ratio of High / Low democracy level")+
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        legend.position = "top")
...
### How does it influence if the protest was violent?
...
ProtestLocationExtracted[!is.na(polity), 
  ...
  geom_errorbarh(aes(xmin = lower_ci, xmax = upper_ci), 
                 height = 0.5, size = 0.5)+
  geom_point(aes(color = protesterviolence),size = 4)+
  geom_vline(aes(xintercept = 1), size = 1)+
  facet_wrap(~protesterviolence, nrow = 1)+
  scale_x_continuous(breaks = breaks_width(0.2))+
  scale_color_manual(values = c("FALSE" = "dodgerblue3", 
                                "TRUE" = "brown2"))+
  labs(title = "State Response vs Polity Level",
